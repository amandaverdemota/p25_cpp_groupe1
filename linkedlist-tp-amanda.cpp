...
struct Cell {
  int valeur, 
  Cell* suite;// commande que j'ai essayé de trouver pour avoir l'adresse de la case suivante
  Cell(int s,int suite):valeur(v), suite(nouvelleloc){
// je crois que 'pas de delete necessaire à revoir 
  }
};

struct LinkedList {
  Cell* head;
  LinkedList() : locactuelle(nullptr) {}
  };
